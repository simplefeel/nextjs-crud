...
async function deleteUsers(client) {
  try {
    await client.sql`DELETE FROM users WHERE id = 'aadfeaaa-2a1d-4c70-a7f0-0427f7472baa'`;
...
async function seedTranscriptions(client) {
  try {
    await client.sql`CREATE EXTENSION IF NOT EXISTS "uuid-ossp"`;
...
async function updateTranscriptions(client) {
  try {
    // Create the "transcriptions" table if it doesn't exist
    const updateTable = await client.sql`
        ALTER TABLE transcriptions ALTER COLUMN audio_text TYPE TEXT;
    `;

    console.log(`Updated "transcriptions" table`);
